---
title: "Aggrégation simple de classements"
author: "L. Caron, L.-A. Vallière-Lavoie et S. Perreault"
date: "June 16, 2019"
...
Dans le cadre de la fin de semaine d'analytique de sports récemment organisée par des .Layeuriens, nous avons construit une petite routine qui aggrège différents *draft rankings* pour en former un unique.
Pardonnez le manque d'explication lolk.
En passant, allez voir le ShinyApp créé par d'autres participants de la fin de semaine (un truc de tennis).
...
head(temp)[,1:3]
```
Les 14 listes (on en a retiré 2 parce qu'elles concernaient seulement les Européens et les Nord-Américains respectivement) contiennent un classement des 32 meilleurs espoirs (choix de première ronde) pour le repêchage de la LNH selon différents experts.
...
Voici notre repêchage final selon cette métrique.
```
library(tibble)
library(purrr)
library(magrittr)
...
by_player <-
 ...
 LAURENT -- ICI ON FAIT UN CALL À by_player pour montrer par exemple le ranking final.
```

```{r baseline, echo=FALSE}
# SAVE UNE COPIE DE by_player ET ON LA CALL ICI
# RECOPIE LE CALL ICI
```
...
rank.mat[1:5,1:5]
```
Chaque rangé correspond aux prédictions d'un expert.
...
Ce qui se passe après 25 ne fait plus de sens puisqur nos données sont censurées à droite.
...
Nous avons finalement projeté nos données sur une droite (une seule dimension) en *pluggant* cette matrice dans la fonction `cmdscale` (multi-dimensional scaling).
